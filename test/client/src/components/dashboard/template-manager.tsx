...
export function TemplateManager() {
  ...
  const { data: realEstateTemplates, isLoading: isLoadingRealEstate } =
    useQuery({
  ...
  const generateMutation = useMutation({
    mutationFn: ({ templateId, variables, title }: any) =>
      apiRequest("POST", `/api/templates/${templateId}/generate`, {
        variables,
        title,
        test: false,
      }),
    ...
    onError: () => {
      toast({
        ...
        description:
          "Failed to generate video from template. Please try again.",
        variant: "destructive",
      });
      setIsGenerating(false);
    },
  ...
  const getTemplateIcon = (templateName: string) => {
    ...
    if (name.includes("property") || name.includes("tour"))
      return <Home className="w-4 h-4" />;
    if (name.includes("market")) return <TrendingUp className="w-4 h-4" />;
    if (name.includes("agent") || name.includes("introduction"))
  ...
  return (
    <Card data-testid="card-template-manager">
      <CardHeader>
        ...
        <CardDescription>
          Create videos from pre-designed templates optimized for real estate
          marketing
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="browse">
          ...
          <TabsContent value="browse" className="space-y-4">
            {isLoadingTemplates ? (
              ...
              <Alert>
                <AlertDescription>
                  No templates available. Create your first template or check
                  the Real Estate tab for suggestions.
                </AlertDescription>
              </Alert>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                {((templates as any)?.templates || [])?.map(
                  (template: Template) => (
                    <div
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="max-w-2xl">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Fill in the variables to generate your custom
                                video
                              </DialogDescription>
                            </DialogHeader>
          ...
          <TabsContent value="real-estate" className="space-y-4">
            {isLoadingRealEstate ? (
              <div className="text-center py-8">
                <Loader2 className="w-8 h-8 animate-spin mx-auto" />
                <p className="text-sm text-gray-500 mt-2">
                  Loading real estate templates...
                </p>
              </div>
            ) : (realEstateTemplates as any)?.suggestions ? (
              <>
                <Alert>
                  <FileText className="h-4 w-4" />
                  <AlertDescription>
                    These are recommended template structures for real estate
                    videos. Use these as inspiration to create your own custom
                    templates.
                  </AlertDescription>
                </Alert>

                <div className="space-y-4">
                  {((realEstateTemplates as any)?.suggestions || []).map(
                    (template: RealEstateTemplate, index: number) => (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(template.recommended_variables).map(
                              ([key, type]) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                {((realEstateTemplates as any)?.templates || []).map(
                  (template: Template) => (
                    <div
                      key={template.template_id}
                      className="border rounded-lg p-4 space-y-3"
          ...
          <TabsContent value="create" className="space-y-4">
            <div className="space-y-4">
              ...
              <Alert>
                <AlertDescription>
                  Advanced template creation with custom elements and animations
                  coming soon. For now, templates can be created from existing
                  videos.
                </AlertDescription>
              </Alert>
