...
export class HeyGenService {
  ...
  private async makeRequest(endpoint: string, method: 'GET' | 'POST' | 'PUT' = 'GET', body?: any): Promise<any> {
    ...
    if (!response.ok) {
      ...
      console.error(`HeyGen API error at ${endpoint}:`, response.status, errorText);
  ...
  async createTalkingPhotoAvatar(
    ...
    console.log('HeyGen response for talking photo avatar:', JSON.stringify(response));
    return response;
  }
  ...
  async importAvatar(avatarId: string): Promise<any> {
    try {
      ...
      if (avatarsList.data?.avatars) {
        const foundAvatar = avatarsList.data.avatars.find((a: any) => a.avatar_id === avatarId);
        if (foundAvatar) {
      ...
      throw new Error(`Failed to import avatar: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async uploadImage(imageBlob: Blob): Promise<string> {
    ...
    if (result.code === 100 && result.data?.url) {
      ...
      throw new Error(`HeyGen upload failed: ${result.msg || result.message || 'Unknown error'}`);
    }
  }
