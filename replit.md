...
- **Kling AI Motion Video Generation**: Enables users to transform static avatar images into dynamic motion videos using Kling AI's image-to-video API. Features include: prompt-based motion description (e.g., "nodding head gently while smiling"), configurable video duration (5 or 10 seconds), real-time progress tracking with polling, and video preview/download. API keys are encrypted using AES-256-GCM (same pattern as AI provider keys). Located in Avatar Studio with "Add Motion" button on avatar look popup. Endpoints: POST /api/kling/generate-motion, GET /api/kling/status/:taskId, PUT/DELETE /api/kling-preferences. Service: server/services/kling.ts.
