...
- **BHHS Compliance System**: Integrated compliance checking for all social media content to meet Berkshire Hathaway HomeServices brokerage requirements. Features include: automatic detection of advertising content, brokerage name requirement enforcement (must appear in first line of ads), prohibited term detection for non-brokers, and one-click auto-fix to add required BHHS branding. The ComplianceChecker component is integrated into the Content Calendar preview dialog, Social Media Manager, and Post Composer. Backend endpoints: GET/PATCH `/api/compliance/settings`, POST `/api/compliance/check`, POST `/api/compliance/fix`, GET `/api/compliance/guidelines`. Service: server/services/compliance.ts.
